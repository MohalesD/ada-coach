...
| **Mo's manual dry run** (throwaway account → feedback → delete → check admin views) | ✅ **Passed, 2026-09-08.** Playwright E2E still not run — that piece of this gate is separate and unchanged. | **Gated launch, not build**, and the manual half is now cleared. Still do not set `BRIDGE_SHARED_SECRET` in production until the Playwright E2E also passes and the live two-account probe below runs. |
| **Delta audit (OQ-C, PRD Milestone 4)** | ✅ **Read-only half passed, 2026-09-08** — `docs/audits/2026-09-08-bridge-delta-audit.md` (run from the Builder Journal orchestrator session, which has the same Supabase access; independently spot-checked from an Ada session too — both agree). One pre-existing, non-blocking finding surfaced: `anon` holds full table-level grants on `products`/`sessions`/`assumptions`/`model_usage`, inert today because no RLS policy targets `anon`, registered as a hygiene item in `tasks/todo.md`. **Open:** a live two-account probe (real signup, real `handoff`/`unlink` calls, both directions) — the read-only pass makes a cross-user leak structurally implausible but doesn't replace exercising it. | **Gates launch, not build.** |
...
- **OQ-C — read-only half answered, 2026-09-08.** The delta audit's catalog-level pass (RLS, policies, unique constraints, table/column grants, HMAC verification, sprint-id isolation, security advisors) ran against the live DB with the 2026-08-23 audit's method — see `docs/audits/2026-09-08-bridge-delta-audit.md`. Still open: the live two-account probe, before `BRIDGE_SHARED_SECRET` is set in production.
