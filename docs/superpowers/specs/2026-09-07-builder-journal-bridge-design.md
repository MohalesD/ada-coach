...
| **Delta audit (OQ-C, PRD Milestone 4)** | ✅ **Read-only half passed, 2026-09-08** — `docs/audits/2026-09-08-bridge-delta-audit.md`, 9/9, independently re-verified against the live DB and source (not copied from the first pass). **Open:** a live two-account probe (real signup, real `handoff`/`unlink` calls, both directions) — the audit's own catalog reads make a cross-user leak structurally implausible but don't replace exercising it. | **Gates launch, not build.** |
...
- **OQ-C — read-only half answered, 2026-09-08.** The delta audit's catalog-level pass (RLS, policies, unique constraints, column grants, HMAC verification, sprint-id isolation) ran against the live DB with the 2026-08-23 audit's method — see `docs/audits/2026-09-08-bridge-delta-audit.md`, 9/9. Still open: the live two-account probe, before `BRIDGE_SHARED_SECRET` is set in production.
